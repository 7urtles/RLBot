...
def steer_toward_target(bot: PlayerInfo, target: Vec3) -> float:
    relative = relative_location(Vec3(bot.car_contact_point), Orientation(bot.car.physics.rotation), target)
    angle = math.atan2(relative.y, relative.x)
    return limit_to_safe_range(angle)