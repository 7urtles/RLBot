...
class Vec3:
    ...
    # ChickenParm
    def rotate2D(self, angle: 'float', origin=None) -> 'Vec3':
        """Returns vector rotated by an angle in radians around an origin through 2D space.
        Second optional argument is a point to use as an origin Vec3"""
        if origin == None: origin = Vec3(0,0,0)
        self = self - origin
        x = math.cos(angle)
        y = math.sin(angle)
        return Vec3(
    ...
    # ChickenParm
    def ang_center(self: 'Vec3') -> 'float':
        """Returns angle of a vector on the field relative to center of field."""
        return math.atan(self.y/self.x)

    # ChickenParm
    def rel_ang_to(self: 'Vec3', other: 'Vec3', origin: 'Vec3') -> 'float':
        """Returns angle of the vector to another point, using an third vector as an origin point"""
        p1 = (self - origin).normalized()
        p2 = (other - origin).normalized()
        angle = abs(math.atan(p1.y/p1.x)-math.atan(p2.y/p2.x))  
        if origin.x - p1.x < 0: angle *= -1
        return angle